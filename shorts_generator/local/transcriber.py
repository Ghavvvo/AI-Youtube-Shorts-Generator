...
def _transcript_cache_path(media_path: str) -> Path:
    """Return the .json cache path for a media file (conserva word-timestamps)."""
...
def _write_json_cache(media_path: str, transcript: Dict) -> Path:
    ...
    cache_path.write_text(json.dumps(transcript, ensure_ascii=False, indent=1), encoding="utf-8")
    return cache_path
...
def transcribe_local(media_path: str, language: Optional[str] = None) -> Dict:
    ...
    transcribe_kwargs = {
        ...
        "word_timestamps": True,  # → words[] por segmento (subs palabra a palabra)
    }
